...
void Ld_8(unsigned char * A, unsigned char * B, CPU* cpu){
    // Generic function to LD B into A.
    (*A) = (*B);
    (*cpu).tick += 4;
}

void Ld_16(unsigned short * A, unsigned short * B, CPU* cpu){
     // Generic function to LD B into A.
     (*A) = (*B);
     (*cpu).tick += 8;
}
...
#endif
    // Cycles: 8, (Z N H C): - - - -
    unsigned char d8 = mmu_read(mmu,(*cpu).ED);
    Ld_8(&(*cpu).A,&d8, cpu);
...
void Add_16_8(unsigned short* A, unsigned char* B, CPU* cpu) {
